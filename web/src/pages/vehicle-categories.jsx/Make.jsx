...
const Make = () => {
    ...
    const URL = "https://api.mechconnect.app/api/v1/vehicle-makes"
    const [vehicleMakes, setVehicleMakes] = useState([])

    const fetchVehiceMakes = async () => {
        try {
    ...
    return (
        <div className=''>
            <div className='border-b pb-2 flex justify-between items-center'>
                <h1 className='text-lg font-bold'>Vehicle Makes</h1>
                ...
                <table className="w-full">
                    ...
                    <tbody>
                        {vehicleMakes.map((make, index) => {
                            const { name, description, createdAt, created_by, updated_by } = make;
            ...
            <CustomDrawer isOpen={isOpen} >
                <h2 className='text-lg border-b py-2 text-blue-800'>Add vehicle make</h2>

                <div>
                    <form onSubmit={handleSubmit} className=' space-y-4 py-5'>
                        <div className='flex flex-col'>
                            <label >Name</label>
                            <input type="text" name="name" placeholder='Toyota,BMW,mercedes etc..' className='p-2 border outline-none rounded' />
                        </div>
                        <div className='flex flex-col'>
